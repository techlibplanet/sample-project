...
resource "aws_ecs_task_definition" "quizzer-td" {
  ...
  {
    REPOSITORY_URL = "ghcr.io/techlibplanet/sample-project",
    /*aws_ecr_repository.quizzer-ecr-repo.repository_url,*/
...
resource "aws_ecs_service" "quizzer-service" {
  ...
  depends_on = [
    aws_lb_listener.api-listener,
    aws_iam_role_policy_attachment.quizzer-ecs-rpa]
  launch_type = "FARGATE"
  network_configuration {
    security_groups = [
      aws_security_group.quizzer-sg.id]
    subnets = [
